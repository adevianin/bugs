...
class JsonGenomeFactory():
    ...
    def build_genome_from_json(self, genome_json: dict) -> Genome:
        maternal_chromosomes_set = self._build_chromosomes_set(genome_json['maternal'])
        paternal_chromosomes_set = self._build_chromosomes_set(genome_json['paternal'])
        
        return Genome.build(maternal_chromosomes_set, paternal_chromosomes_set)

    def _build_chromosomes_set(self, chromosomes_set_json: dict) -> ChromosomesSet:
