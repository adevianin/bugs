...
from core.world.exceptions import AccessDeniedError, EntityNotFoundError, GameError
...
class World():
    ...
    def find_ant_for_owner(self, ant_id: int, owner_id: int) -> Ant: 
        ant: Ant = self.map.get_entity_by_id(ant_id)
    ...
    def find_nest_for_owner(self, nest_id: int, owner_id: int) -> Nest:
        nest: Nest = self.map.get_entity_by_id(nest_id)
    ...
    def find_ant_colony_for_owner(self, colony_id: int, owner_id: int) -> AntColony:
        colony: AntColony = self.get_colony_by_id(colony_id)
