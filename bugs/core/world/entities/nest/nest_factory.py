...
class NestFactory():
    ...
    def build_new_nest(self, id: int, position: Point, ownership: OwnershipConfig):
        return self.build_nest(id=id, position=position, angle=0, hp=100, ownership=ownership, larvae=[], eggs=[], stored_calories=0, area=300, build_progress=0,
                               fortification=0)
    ...
    def build_nest(self, id: int, position: Point, angle: int, ownership: OwnershipConfig, hp: int, larvae: List[Larva], eggs: List[Egg], stored_calories: int, area: int, 
        ...
        return Nest(self._event_bus, EventEmitter(), id, ownership, body)
