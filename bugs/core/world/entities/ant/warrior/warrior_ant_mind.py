...
class WarrirorAntMind(AntMind):
    ...
    def patrolling_territory(self, sayback: str = None):
        thought = self._thought_factory.build_patrolling_nest_territory_full(self.home_nest)
        self._register_thought(thought)

    def _generate_thoughts(self):
        super()._generate_thoughts()
        if not self._has_thoughts_to_do():
            self.patrolling_territory()
