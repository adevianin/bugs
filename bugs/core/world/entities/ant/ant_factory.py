...
class AntFactory():
    ...
    def build_ant(self, id: int, from_colony: int, ant_type: AntTypes, dna_profile: str, position: Point, nest: Nest, located_in_nest: Nest, memory: Memory) -> Ant:
        match ant_type:
            case AntTypes.WORKER:
                return self._build_worker_ant(id, from_colony, dna_profile, position, nest, located_in_nest, memory)
            case AntTypes.WARRIOR:
                return self._build_warrior_ant(id, from_colony, dna_profile, position, nest, located_in_nest, memory)
            case AntTypes.QUEEN:
                return self._build_queen_ant(id, from_colony, dna_profile, position, nest, located_in_nest, memory)
            case _:
                raise Exception('uknown type of ant')
            
    def _build_warrior_ant(self, id: int, from_colony: int, dna_profile: str, position: Point, nest: Nest, located_in_nest: Nest, memory: Memory):
        events = EventEmitter()
    ...
    def _build_worker_ant(self, id: int, from_colony: int, dna_profile: str, position: Point, nest: Nest, located_in_nest: Nest, memory: Memory):
        events = EventEmitter()
        world_interactor = WorldInteractor()
        memory = memory if memory else Memory()
    ...
    def _build_queen_ant(self, id: int, from_colony: int, dna_profile: str, position: Point, nest: Nest, located_in_nest: Nest, memory: Memory):
        events = EventEmitter()
        world_interactor = WorldInteractor()
        memory = memory if memory else Memory()
