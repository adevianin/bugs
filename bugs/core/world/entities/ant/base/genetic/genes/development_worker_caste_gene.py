...
class DevelopmentWorkerCasteGene(DevelopmentCasteGene):
    ...
    @classmethod
    def build(cls, domination_code: DominationCodes, strength: int, defense: int, max_hp: int, hp_regen_rate: int, speed: int) -> 'DevelopmentWorkerCasteGene':
    ...
    def build_new_for_specie_gene():
        return DevelopmentWorkerCasteGene.build(DominationCodes.random(), 1, 1, 1, 1, 1)

    def __init__(self, domination_code: DominationCodes, strength: int, defense: int, max_hp: int, hp_regen_rate: int, speed: int):
        super().__init__(GenesTypes.DEVELOPMENT_WORKER_CASTE, domination_code, strength, defense, max_hp, hp_regen_rate, speed, True)
