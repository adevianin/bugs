...
from core.world.settings import NUPT_MALE_SUPER_GENE_UPGRADE_MULTIPLIER, BASE_BODY_HP_REGEN_RATE
from core.world.entities.ant.base.genetic.chromosome_types import ChromosomeTypes

class BodyHpRegenRateGene(BaseGene):

    @classmethod
    def build(cls, domination_code: DominationCodes, hp_regen_rate: float):
        return BodyHpRegenRateGene(domination_code, hp_regen_rate)
    
    @staticmethod
    def build_new_for_specie_gene():
        return BodyHpRegenRateGene.build(DominationCodes.random(), BASE_BODY_HP_REGEN_RATE)

    def __init__(self, domination_code: DominationCodes, hp_regen_rate: float):
    ...
    def upgrade(self) -> 'BodyHpRegenRateGene':
        return BodyHpRegenRateGene.build(DominationCodes.random(), self._hp_regen_rate * NUPT_MALE_SUPER_GENE_UPGRADE_MULTIPLIER)
