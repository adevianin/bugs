...
class BirtherService():
    ...
    def _on_preborn_ant(self, larva: Larva, nest: Nest):
        new_ant = self._ant_factory.build_ant(None, nest.from_colony, larva.ant_type, larva.dna_profile, larva.position, nest, None, None)
        self._world.map.give_birth_entity(new_ant)

    def _on_preborn_food(self, preborn_food: PrebornFood):
        new_food = self._food_factory.build_food(None, preborn_food.position, preborn_food.calories, preborn_food.food_type, -1)
