...
def found_colony(request: HttpRequest):
    ...
    ea.found_colony_command(request.user.id, queen_id, nuptial_male_id, nest_building_site, colony_name)
...
def save_specie_schema(request: HttpRequest):
    ...
    ea.change_specie_schema_command(request.user.id, specie_schema)
...
def born_new_antara(request: HttpRequest):
    ...
    ea.born_new_antara_command(request.user.id)
