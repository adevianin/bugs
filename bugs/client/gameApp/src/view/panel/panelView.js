...
class PanelView extends BaseHTMLView {
    ...
    _renderTabViews() {
        ...
        this._tabSwitcher = new TabSwitcher(this._el.querySelector('[data-tab-switcher]'), 'panel', [
            { name: 'breeding', label: MessageMaster.get(MESSAGE_IDS.TAB_BREEDING), tab: this._nuptialFlightTab },
            { name: 'colonies', label: MessageMaster.get(MESSAGE_IDS.TAB_COLONIES), tab: this._coloniesTab },
            { name: 'specie_builder', label: MessageMaster.get(MESSAGE_IDS.TAB_SPECIE), tab: this._specieBuildertTab },
            { name: 'notifications', label: MessageMaster.get(MESSAGE_IDS.TAB_NOTIFICATIONS), tab: this._notificationsTab },
            { name: 'rating', label: MessageMaster.get(MESSAGE_IDS.TAB_RATING), tab: this._ratingTab },
            { name: 'user', label: MessageMaster.get(MESSAGE_IDS.TAB_ACCOUNT), tab: this._userTab }
        ]);
    }
